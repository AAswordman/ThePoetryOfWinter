...
class PomTalentSystem extends GameController {
    ...
    updateArmorData() {
        ...
        this.armor_movement_addition = ["movement_addition", "sneak_movement_addition", "underwater_movement_addition"]
            .map(e => (this.headComp?.getComponentWithGroup(e) ?? 0)
            + (this.chestComp?.getComponentWithGroup(e) ?? 0)
            + (this.legComp?.getComponentWithGroup(e) ?? 0)
            + (this.feetComp?.getComponentWithGroup(e) ?? 0));
        ...
        this.getEvents().exEvents.afterItemOnHandChange.subscribe((e) => {
            ...
            if (e.afterItem) {
                ...
                if (comp.hasComponent("armor_protection"))
                    base.push("§r§7•护甲值§6+" + comp.getComponentWithGroup("armor_protection") + "§r§7 | 护甲韧性§6+" + comp.getComponentWithGroup("armor_resilience") ?? 0);
                if (comp.hasComponent("armor_type")) {
                    ...
                    if (comp.hasComponent("armor_physical_protection"))
                        base.push("§r§7•物理抗性§6+" + comp.getComponentWithGroup("armor_physical_protection") + "％§r§7 | 受到的物理伤害§6-" + comp.getComponentWithGroup("armor_physical_reduction") ?? 0);
                    if (comp.hasComponent("armor_magic_protection"))
                        base.push("§r§7•魔法抗性§6+" + comp.getComponentWithGroup("armor_magic_protection") + "％§r§7 | 受到的魔法伤害§6-" + comp.getComponentWithGroup("armor_magic_reduction") ?? 0);
                }
