...
execute if score IsNight global = one global if score NightRandom global = one global run tellraw @s { "rawtext" : [ { "translate" : "text.dec:everlasting_winter_heart_already.name" } ] }
execute if score IsNight global = one global unless score NightRandom global = one global run tag @s add everlasting_winter_heart_skill
...
execute if entity @s[tag=everlasting_winter_heart_skill] run scoreboard players set @a night_event 4
